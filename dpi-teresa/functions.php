...
dpi_create_widget_area('Featured Links', 'featured_links_widget', 'Featured links area', 'featured-links');
...
function dpi_homepage_link_boxes() {
    //the query
    $the_query = new WP_Query( array( 'post_type' => 'link-boxes', 'posts_per_page' => 3 ) );
	...
	while ( $the_query->have_posts() ) {
		...
		$imageURL = wp_get_attachment_thumb_url( get_post_thumbnail_id( $post->ID ) );
			...
			$string .= '<a href="' . link_box_details_get_meta( 'link_box_details_click_url' ) . '"><span class="title">' . link_box_details_get_meta( 'link_box_details_title' ) . '</span><img src="' . $imageURL . '"></a>';
...
// adds custom meta boxes to admin ui when adding/editing link box post
function dpi_link_boxes__meta_image() {

	remove_meta_box( 'postimagediv', 'link-boxes', 'side' );

	add_meta_box( 'postimagediv', 'Link Box Image', 'post_thumbnail_meta_box', 'link-boxes', 'normal', 'high' );

}

function dpi_link_boxes_meta_title() {

	add_meta_box( 'dpi_link_box_title_options', 'Link Box Title', 'dpi_link_box_title_options_meta_box_html', 'link-boxes', 'normal', 'core' );

}

add_action( 'do_meta_boxes', 'dpi_link_boxes__meta_image' );
...
// remove post published/view post message in admin ui
...
/**** DPI HOMEPAGE EVENTS POSTS ****/




//HOMEPAGE events posts
function dpi_homepage_events_posts() {
// the query
$the_query = new WP_Query( array( 'category_name' => 'events', 'posts_per_page' => 4 ) );

// shortcode output
if ( $the_query->have_posts() ) {
	$string .= '<ul class="homepage-event-posts event-posts">';
	while ( $the_query->have_posts() ) {
		$the_query->the_post();
		$imageUrl;
		...
		if ( has_post_thumbnail() ) {
			...
			$imageUrl = 'http://mtctampa.diocesanweb.com/wp/wp-content/uploads/2016/07/mt-placeholder-img-2.jpg';
		}
            $string .= '<li>';
...
// Add a shortcode
add_shortcode('homepage_events_posts', 'dpi_homepage_events_posts');
        ...
        ?>
            <div class="staff_member">
                ...
                <div class="staff_contacts">
                    <?php if( !empty( $email ) ) : ?>
                        <p class="staff_email"><a href="mailto:<?php echo $email; ?>"><img src="/wp/wp-content/themes/dpi-teresa/images/icons/close-envelope-black.png" /><?php echo $email; ?></a></p>
                    <?php endif; ?>
                    <?php if( !empty( $phone ) ) : ?>
                        <p class="staff_phone"><a href="tel:+<?php echo $phone; ?>"><img src="/wp/wp-content/themes/dpi-teresa/images/icons/telephone-black.png" /><?php echo $phone; ?></a></p>
                    <?php endif; ?>
                </div>
            </div>
        <?php
...
// Add a shortcode
add_shortcode('events_posts', 'wpb_postsbycategory2');
