...
const env = {
  ...
  devPath: 'C:/Users/DPI/Desktop/dev/wordpress/sfa-yulee/wp-content/themes',
  devUrl: 'localhost'
}
...
*/
gulp.task('assets', () => {
  const icons = gulp.src('./source/assets/icons/*.+(jpg|jpeg|gif|png)')
                    .pipe(imagemin())
                    .pipe(gulp.dest(path.resolve(env.distPath + '/assets/icons')));

  const images = gulp.src('./source/assets/images/*.+(jpg|jpeg|gif|png)')
                      .pipe(imagemin())
...
*/
gulp.task('migrate', () => gulp.src('./source/**/*+(screenshot.png|.php|.es5.js|.svg)', {base: './source'})
    .pipe(gulp.dest(path.resolve(env.distPath)))
);
